...
export const useLearningContexts = () => {
  ...
  const getContext = (id: string) =>
    useApiQuery<{ success: boolean; data: LearningContext }>(["learning-context", id], {
      url: `/learning-context/${id}`,
      method: "GET",
    });

  const createContext = useApiMutation<{ success: boolean; data: LearningContext }, CreateLearningContextDto>({
    url: "/learning-context",
    method: "POST",
  });

  const updateContext = (id: string) =>
    useApiMutation<{ success: boolean; data: LearningContext }, UpdateLearningContextDto>({
      url: `/learning-context/${id}`,
      method: "PATCH",
    });
  ...
  const getContextProgress = (contextId: string) =>
    useApiQuery<{ success: boolean; data: LearningProgress }>(["learning-progress", contextId], {
  ...
  const switchContext = useApiMutation<{ success: boolean }, { contextId: string }>({
